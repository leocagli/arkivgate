...
We expose two shapes:
  - JSON Message â€” for clients that don't request streaming.
  - SSE event sequence â€” for clients that pass `stream: true` (Claude Code's
...
def _block_text(hit: PolicyHit) -> str:
    return (
        ...
        f"Tu mensaje se cruza con la polÃ­tica **{hit.slug}** de tu organizaciÃ³n:\n\n"
        f"> {hit.rule}\n\n"
        f"La idea no es frenarte sino asegurarnos de que el entregable quede alineado "
        f"con lo que la empresa espera. Si reformulÃ¡s el prompt sin incluir esa informaciÃ³n, "
        f"puedo procesarlo normalmente. Si el contexto es imprescindible para tu tarea, "
        f"coordinÃ¡ con tu admin.\n\n"
        f"Â¿QuerÃ©s que te ayude a reformularlo? Contame quÃ© estÃ¡s intentando hacer "
        f"y lo armamos juntos respetando la polÃ­tica.\n\n"
        f"â€” ArkivGate"
    )
...
def synthesize_block_message(model: str, trace_id: str, hit: PolicyHit) -> dict:
    return {
        "id": f"msg_ArkivGate_blocked_{trace_id}",
        "type": "message",
        "role": "assistant",
        "model": model,
        "content": [{"type": "text", "text": _block_text(hit)}],
        "stop_reason": "ArkivGate_blocked",
        "stop_sequence": None,
...
async def synthesize_block_sse(
    ...
    msg_id = f"msg_ArkivGate_blocked_{trace_id}"
    ...
    yield _sse(
        ...
        {
            "type": "message_delta",
            "delta": {"stop_reason": "ArkivGate_blocked", "stop_sequence": None},
